...
if page == "🔄 Synchronisation":
    ...
    with col2:
        ...
        if st.button("🔄 Backfill TVA Etsy", use_container_width=True):
            with st.spinner("Backfill tva_etsy sur tout l'historique Etsy... (peut être long)"):
                debut = time.time()
                try:
                    shop_id_etsy = get_shop_id()
                    if shop_id_etsy:
                        nb_traitees, nb_maj = backfill_etsy_tva(shop_id_etsy)
                        duree = time.time() - debut
                        log_sync_etsy("backfill_tva_etsy", nb_maj, "success",
                                     f"{nb_maj}/{nb_traitees} commandes", duree)
                        st.success(f"✓ {nb_maj} commandes mises à jour sur {nb_traitees} traitées en {duree:.1f}s")
                    else:
                        st.error("Impossible de récupérer le shop_id Etsy.")
                except Exception as e:
                    st.error(f"Erreur : {e}")
